...
SCHEMA_VERSION = "2.1"
ALLOWED_SIGNALS = {"VETO", "AVOID", "WATCHLIST", "READY_WAIT_TRIGGER", "TRIGGER_CONFIRMED"}
ALLOWED_QUOTE_STATUS = {"VALID", "INVALID", "STALE_POSSIBLE", "MISSING", "WIDE_OR_INCOHERENT"}
ALLOWED_QUOTE_QUALITY = {"HIGH", "MEDIUM", "LOW"}
ALLOWED_RUN_TRUST = {"TRUSTED", "DEGRADED", "UNUSABLE"}
REQUIRED_COLUMNS = {
    "ticker", "signal", "final_score", "final_trade_score", "quote_status",
    "execution_quote_quality", "all_veto_reasons", "penalty_reasons",
    "actionable_entry", "actionable_stop", "actionable_target",
    "theoretical_entry", "theoretical_stop", "theoretical_target",
    "legacy_rank", "trade_rank", "rank_delta", "run_trust_status",
    "run_trust_reasons", "critical_essential_sources", "run_manual_review_required",
}
...
def validate_scan_schema(df: pd.DataFrame) -> list[SchemaViolation]:
    ...
    for column, allowed, code in (
        ("signal", ALLOWED_SIGNALS, "invalid_signal"),
        ("quote_status", ALLOWED_QUOTE_STATUS, "invalid_quote_status"),
        ("execution_quote_quality", ALLOWED_QUOTE_QUALITY, "invalid_quote_quality"),
        ("run_trust_status", ALLOWED_RUN_TRUST, "invalid_run_trust"),
    ):
        invalid = sorted(set(df[column].dropna()) - allowed)
        if invalid:
            violations.append(SchemaViolation(code, ", ".join(map(str, invalid))))
