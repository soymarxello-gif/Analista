...
from tools.gui_weekly_operational_review import MANUAL_NOTICE, NOTICE, save_review

DEFAULT_JSON_OUT = ROOT / "reports" / "gui_weekly_operational_review_audit_latest.json"
DEFAULT_MARKDOWN_OUT = ROOT / "reports" / "gui_weekly_operational_review_audit_latest.md"
...
def _review_source_terms() -> dict[str, list[str]]:
    return {
...
def run_audit(
    *,
    ...
    if root != ROOT and json_out == DEFAULT_JSON_OUT:
        json_out = root / "reports" / "gui_weekly_operational_review_audit_latest.json"
    if root != ROOT and markdown_out == DEFAULT_MARKDOWN_OUT:
        markdown_out = root / "reports" / "gui_weekly_operational_review_audit_latest.md"

    tool_path = ROOT / "tools" / "gui_weekly_operational_review.py"
    ...
    review = save_review(root=root)
    protected_after = {
        "decisions": _sha(data / "gui_operational_decisions.csv"),
        "journal": _sha(data / "paper_trading_journal.csv"),
        "outcomes": _sha(data / "trade_outcomes.csv"),
    }
    json_report = reports / "gui_weekly_operational_review_latest.json"
    md_report = reports / "gui_weekly_operational_review_latest.md"
    csv_report = reports / "gui_weekly_operational_review_latest.csv"
    history_csv = data / "gui_weekly_operational_reviews.csv"
    source = _read_text(tool_path)
    md_text = _read_text(md_report)

    terms = _review_source_terms()
    ...
    add("protected_data_unchanged", protected == protected_after)
    ...
    add("manual_review_only", bool(review.get("manual_review_only", False)))
    add("paper_trading_only", bool(review.get("paper_trading_only", False)))
    add("no_real_order_notice", bool(review.get("no_real_order_notice_present", False)))
    ...
    result = {
        ...
        "warnings": 0,
        "weekly_operational_score": review.get("weekly_operational_score"),
        "weekly_operational_bucket": review.get("weekly_operational_bucket"),
        "weekly_recommendation": review.get("weekly_recommendation"),
        "manual_review_only": bool(review.get("manual_review_only", False)),
        "paper_trading_only": bool(review.get("paper_trading_only", False)),
        "no_real_order_notice": bool(review.get("no_real_order_notice_present", False)),
        ...
        "protected_data_unchanged": protected == protected_after,
        "broker_connection_detected": bool(api_hits),
...
def build_markdown(data: dict) -> str:
    lines = [
        "# Analista - GUI weekly operational review audit",
        "",
        f"- status: {data.get('status')}",
        f"- critical_failures: {data.get('critical_failures', 0)}",
        f"- warnings: {data.get('warnings', 0)}",
        f"- weekly_operational_score: {data.get('weekly_operational_score')}",
        f"- weekly_operational_bucket: {data.get('weekly_operational_bucket')}",
        f"- weekly_recommendation: {data.get('weekly_recommendation')}",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Audita GUI weekly operational review.")
    parser.add_argument("--json-out", default=str(DEFAULT_JSON_OUT))
    parser.add_argument("--markdown-out", default=str(DEFAULT_MARKDOWN_OUT))
    args = parser.parse_args()
    result = run_audit(json_out=Path(args.json_out), markdown_out=Path(args.markdown_out))
    print("=== ANALISTA GUI WEEKLY OPERATIONAL REVIEW AUDIT ===")
