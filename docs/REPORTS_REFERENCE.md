...
### `reports/paper_trading_cycle_audit_latest.*`

End-to-end audit of the paper trading cycle. It checks journal structure, open/closed/exported paper trades, pending exports, outcome export correspondence, duplicate `source_journal_id` values, calibration/recommendation status, and broker/order guardrails.

The report is read-only and does not modify the paper journal, trade outcomes, scanner, scoring, config, weights, thresholds, or signals.

### `reports/ui_data_contract_audit_latest.*`

Audit of the read-only data contract for a future GUI. It verifies `ui/report_loader.py`, `ui/view_models.py`, controlled missing/invalid/empty statuses, candidate table model readiness, paper trading summaries, cycle audit summaries, calibration summaries, and guardrails against operational actions.
...
This report is observational only. It does not write scanner data, does not modify the paper journal or trade outcomes, does not run the scanner, does not connect to a broker, and does not send orders.

### `reports/gui_actions_audit_latest.*`

Audit of controlled paper-trading actions exposed by the Streamlit dashboard. It verifies that `app.py` routes actions through `ui/actions.py`, that explicit confirmations exist, that no shell execution is allowed, and that order/broker guardrails remain intact.

This report is observational only. It does not modify the paper journal, trade outcomes, scanner, scoring, config, weights, thresholds, or signals.
