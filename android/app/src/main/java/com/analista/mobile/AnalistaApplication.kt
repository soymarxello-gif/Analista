...
class AnalistaApplication : Application() {
    val database by lazy { Room.databaseBuilder(this, AnalistaDatabase::class.java, "analista.db").fallbackToDestructiveMigration().build() }
    val repository by lazy { ScanRepository(database.dao(), YahooFinanceClient()) }
}
